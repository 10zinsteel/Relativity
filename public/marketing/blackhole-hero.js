/* ============================================================================
   BLACK HOLE HERO — vanilla-JS renderer
   ============================================================================
   Ported from a React/WebGL hero component. The physics and shaders are
   plain GLSL and framework-agnostic; only the mount/lifecycle wiring changed
   to work as a script tag against a canvas + host element instead of a
   React effect.

   One ray per pixel is fired from the camera and walked backwards through
   curved space (Schwarzschild geodesics) until it falls through the horizon,
   escapes to the stars, or crosses the gas disc — often several times, since
   a ray can loop the hole and come back. That's why the halo above the disc,
   the halo below it, and the thin ring hugging the shadow all line up: they
   are the same disc, seen again through bent light.

   Units are set by the horizon: r = 1. The photon sphere sits at 1.5, the
   shadow is 2.6 across, and the disc starts at 3 (innermost stable orbit).
   Those numbers fall out of the metric; they are not tuned.
   ============================================================================ */
...
`;
  ...
  /**
   * Mounts the renderer onto `canvas`, sized against `host`.
   * Returns { setOptions(patch), destroy() }.
   */
  function initBlackHoleHero(host, canvas, overrides) {
    ...
    function dropTargets() {
      ...
      scene = null; histA = null; histB = null; bloomA = null; bloomB = null;
      settled = 0;
    }

    function resize() {
      const rect = host.getBoundingClientRect();
      const dpr = software ? 1 : Math.min(window.devicePixelRatio || 1, Math.max(1, options.maxDpr));
      const cssW = Math.max(1, Math.round(rect.width));
      const cssH = Math.max(1, Math.round(rect.height));
      const scale = software ? 0.34 : Math.min(1, Math.max(0.4, options.resolution));
    ...
    function tick(now) {
      ...
      lastFrame = now;
      if (!options.paused && !reduced) clock += dt;
      render(clock);
    }
    ...
    const ro = new ResizeObserver(() => {
      resize();
      if (reduced || options.paused) settle(16);
    });
    ro.observe(host);
    ...
    return {
      setOptions(patch) {
        ...
        if (reduced || options.paused) settle(4);
      },
      destroy() {
        running = false;
        cancelAnimationFrame(raf);
  ...
  window.RelativityBlackHole = { init: initBlackHoleHero };
})();
